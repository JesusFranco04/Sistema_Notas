...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($nombre) && !empty($abreviatura) && !empty($fecha_ingreso)) {

        // Escapar caracteres especiales y evitar inyección SQL
        $nombre = mysqli_real_escape_string($conn, $nombre);
        $abreviatura = mysqli_real_escape_string($conn, $abreviatura);
        ...
        $sql = "INSERT INTO subniveles (nombre, abreviatura, fecha_ingreso)
                VALUES ('$nombre','$abreviatura', '$fecha_ingreso')";
                
        if (mysqli_query($conn, $sql)) {
            header("Location: ../../views/admin/subnivel.php");
...
<head>
    ...
    <title>Agregar Periodo| Sistema de Gestión UEBF</title>
    ...
    <div class="container mt-5">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title">Agregar Subnivel</h5>
            </div>
            <div class="card-body">
                <form action="agregar_subnivel.php" method="post" enctype="multipart/form-data"
                    ...
                    <div class="form-group">
                        <label for="abreviatura">abreviatrura:</label>
                    ...
                    <a href="../../views/admin/subnivel.php" class="btn btn-secondary">Regresar</a>
                </form>
            </div>
        </div>
    </div>
