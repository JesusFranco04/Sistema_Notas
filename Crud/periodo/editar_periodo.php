...
<div>
    <form method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
        ...
        <input type="text" id="ano" name="ano" value="<?php echo $fila['ano']; ?>" required><br>
        <label for="fecha_ingreso">Fecha de ingreso:</label>
        <input type="text" id="fecha_ingreso" name="fecha_ingreso" value="<?php echo date('Y-m-d H:i:s'); ?>"
            readonly><br>
        <input type="submit" name="actualizar" value="Actualizar">
        <a href="../../views/admin/periodo.php">Regresar</a>
    </form>
</div>
<?php
    } else {
...
if (isset($_POST['actualizar'])) {
    ...
    if (!empty($ano) && !empty($fecha_ingreso)) {
        ...
        $sql2 = "UPDATE periodo SET ano=?, fecha_ingreso=? WHERE id=?";
        ...
        // Vincular parámetros
        mysqli_stmt_bind_param($stmt, "ssi", $ano, $fecha_ingreso, $idp);
...
<head>
    ...
    <title>Editar Periodo| Sistema de Gestión UEBF</title>
