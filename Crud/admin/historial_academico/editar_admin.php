...
<div>
    <form method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
        ...
        <select id="rol" name="rol" required>
            <option value="1" <?php if ($fila['rol'] == '1') echo 'selected'; ?>>Profesor</option>
        </select><br>
        ...
        <a href="../../views/admin/administradores.php">Regresar</a>
    </form>
</div>
<?php
    } else {
...
if (isset($_POST['actualizar'])) {
    ...
    if (!empty($nombres) && !empty($apellidos) && !empty($cedula) && !empty($telefono) && !empty($correo_electronico) && !empty($direccion) && !empty($fecha_nacimiento) && !empty($genero) && !empty($discapacidad) && !empty($rol) && !empty($date_creation)) {
        ...
        $sql2 = "UPDATE administrador SET nombres=?, apellidos=?, cedula=?, telefono=?, correo_electronico=?, direccion=?, fecha_nacimiento=?, genero=?, discapacidad=?, rol=?, date_creation=? WHERE id=?";
...
<head>
    ...
    <title>Editar admin| Sistema de Gestión UEBF</title>
