...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM subniveles WHERE id='" . $id . "'";
...
<div>
    <form method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
        ...
        <input type="text" id="abreviatura" name="abreviatura" value="<?php echo $fila['abreviatura']; ?>" required><br>
        <label for="fecha_ingreso">Fecha de ingreso:</label>
        <input type="text" id="fecha_ingreso" name="fecha_ingreso" value="<?php echo date('Y-m-d H:i:s'); ?>"
            readonly><br>
        <input type="submit" name="actualizar" value="Actualizar">
        <a href="../../views/admin/subnivel">Regresar</a>
    </form>
</div>
<?php
    } else {
...
if (isset($_POST['actualizar'])) {
    ...
    if (!empty($nombre)  && !empty($abreviatura) && !empty($fecha_ingreso)) {
        ...
        $sql2 = "UPDATE subniveles SET nombre=?, abreviatura=?,  fecha_ingreso=? WHERE id=?";
        ...
        // Vincular parámetros
        mysqli_stmt_bind_param($stmt, "sssi", $nombre, $abreviatura, $fecha_ingreso, $idp);
...
<head>
    ...
    <title>Editar Subnivel| Sistema de Gestión UEBF</title>
