...
<div>
    <form method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
        ...
        <select id="rol" name="rol" required>
            <option value="3" <?php if ($fila['rol'] == '3') echo 'selected'; ?>>Padre de familia</option>
        </select><br>
        ...
        <a href="../../views/admin/vistaspadres_admin.php">Regresar</a>
    </form>
</div>
<?php
    } else {
        echo "Error al obtener los datos del pradres.";
    }
...
if (isset($_POST['actualizar'])) {
    ...
    if (!empty($nombres) && !empty($apellidos) && !empty($cedula) && !empty($telefono) && !empty($correo_electronico) && !empty($direccion) && !empty($fecha_nacimiento) && !empty($genero) && !empty($discapacidad) && !empty($rol) && !empty($date_creation)) {
        ...
        $sql2 = "UPDATE padres SET nombres=?, apellidos=?, cedula=?, telefono=?, correo_electronico=?, direccion=?, fecha_nacimiento=?, genero=?, discapacidad=?, rol=?, date_creation=? WHERE id=?";
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            header("Location: ../../views/admin/vistaspadres_admin.php");
            exit();
        } else {
...
<head>
    ...
    <title> editar padres | Sistema de Gestión UEBF</title>
