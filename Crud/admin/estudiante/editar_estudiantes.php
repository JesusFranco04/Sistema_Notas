...
<div>
    <form method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
        ...
        <a href="../../views/admin/vistaestudiante_admin.php">Regresar</a>
    </form>
</div>
<?php
    } else {
...
if (isset($_POST['actualizar'])) {
    ...
    if (!empty($nombres) && !empty($apellidos) && !empty($cedula) && !empty($telefono) && !empty($correo_electronico) && !empty($direccion) && !empty($fecha_nacimiento) && !empty($genero) && !empty($discapacidad) && !empty($date_creation)) {
        // Conectar a la base de datos (usando mysqli)
        $conn = mysqli_connect($servername, $username, $password, $dbname);
        ...
        $sql2 = "UPDATE estudiantes SET nombres=?, apellidos=?, cedula=?, telefono=?, correo_electronico=?, direccion=?, fecha_nacimiento=?, genero=?, discapacidad=?, date_creation=? WHERE id=?";
        ...
        // Vincular parámetros
        mysqli_stmt_bind_param($stmt, "ssssssssssi", $nombres, $apellidos, $cedula, $telefono, $correo_electronico, $direccion, $fecha_nacimiento, $genero, $discapacidad, $date_creation, $idp);
...
<head>
    ...
    <title>Editar estudiantes | Sistema de Gestión UEBF</title>
