...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Verificar si el jornada ya existe
    $query_check = "SELECT COUNT(*) as total FROM jornada WHERE nombre = ?";
    ...
    if ($row['total'] > 0) {
        $error = "El jornada '$nombre' ya está registrado.";
    } else {
        // Insertar en la base de datos
        $query_insert = "INSERT INTO jornada (nombre, estado, usuario_ingreso, fecha_ingreso) 
                         VALUES (?, ?, ?, ?)";
        $stmt_insert = $conn->prepare($query_insert);
        $stmt_insert->bind_param("ssss", $nombre, $estado, $usuario_ingreso, $fecha_ingreso);

        if ($stmt_insert->execute()) {
            $success = "El jornada '$nombre' ha sido creado exitosamente.";
        } else {
            $error = "Error al crear el jornada. Inténtalo nuevamente.";
        }
    }
}
            ...
            <form action="" method="POST" onsubmit="return validarFormulario()">
                ...
                <div class="button-group mt-4">
                    ...
                    <button type="submit" class="btn btn-crear-usuario">
                        <i class='bx bx-save'></i> Crear Jornada
                    </button>
                </div>
            </form>
        </div>
