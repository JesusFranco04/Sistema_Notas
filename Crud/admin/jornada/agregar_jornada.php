...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($nombre) && !empty($estado)) {
        try {
            ...
            $query_check = "SELECT * FROM jornada WHERE nombre = ?";
            ...
            if ($result_check->num_rows > 0) {
                ...
                $query_insert = "INSERT INTO jornada (nombre, estado, usuario_ingreso, fecha_ingreso) VALUES (?, ?, ?, ?)";
                $stmt_insert = $conn->prepare($query_insert);
                $stmt_insert->bind_param("ssss", $nombre, $estado, $usuario_ingreso, $fecha_ingreso);

                if ($stmt_insert->execute()) {
                    $success = "La jornada '$nombre' ha sido creada exitosamente.";
                } else {
                    $error = "Error al crear la jornada. Inténtalo nuevamente.";
                }
            }
        } catch (Exception $e) {
...
<head>
    ...
    <title>Registro de Jornadas | Sistema de Gestión UEBF</title>
...
<body>
    <div class="header-banner">
        <h1>Formulario de Registro de Jornadas | Sistema de Gestión UEBF</h1>
    </div>
    <div class="container">
        <h2><i class='bx bxs-folder-plus'></i> Registro de Jornada</h2>
        <div class="card-body">
            <?php
            ...
            <form action="" method="POST" onsubmit="return validarFormulario()">
                <div class="mb-3">
                    <label for="nombre" class="form-label required"><i class='bx bxs-time'></i> Nombre:</label>
                ...
                <div class="button-group mt-4">
                    ...
                    <button type="submit" class="btn btn-primary"><i class='bx bx-save'></i> Crear Jornada</button>
                </div>
            </form>
        </div>
    </div>
